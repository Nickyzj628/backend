...
const compressImage = async (src: string, options: CompressionOptions = {}) => {
	return new Promise<string>((resolve, reject) => {
		const img = new Image();

		img.onload = async () => {
			const { maxWidth, maxHeight, maxSizeKB, quality = 0.92 } = options;
			...
			if (maxSizeKB) {
				...
				if (base64SizeKB > maxSizeKB) {
					...
					while (
						(lowBase64.length * (3 / 4)) / 1024 > maxSizeKB &&
						lowQuality > 0
					) {
						lowQuality -= 0.1;
