import type { CreateRoomResponse, RoomMessage, RoomMessagePayload } from "@nickyzj/shared-types";
...
const Room = ({ isHost = true, onChangeHost = (isHost: boolean) => void 0 }) => {
  ...
  useEffect(() => {
    const onRoomCreated = (payload: CreateRoomResponse) => {
      ...
      toast.success(
        <button className="text-left" onClick={() => copyToClipboard(window.location.href)}>
          房间创建成功！点击复制链接到剪贴板
        </button>,
        { duration: 10000 },
      );
