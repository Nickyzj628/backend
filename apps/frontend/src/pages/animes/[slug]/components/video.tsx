import type { Anime, RoomMessage, VideoInfo, VideoSyncRequestPayload } from "@nickyzj/shared-types";
...
const Video = ({ anime, isHost = true }: Props) => {
  ...
  const loadVolume = () => {
    videoRef.current.volume = parseFloat(localStorage.getItem("volume") ?? "0.5");
  };
  ...
  return (
    <div ref={containerRef} className="relative aspect-video w-full xl:flex-1 rounded-xl">
      <video
        ref={videoRef}
