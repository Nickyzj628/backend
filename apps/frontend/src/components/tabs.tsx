...
Tabs.List = ({ className, children, onChange }: TabsListProps) => {
  ...
  return (
    <div
      className={clsx(
        "flex shrink-0 gap-1 p-1 rounded-xl bg-neutral-100 overflow-x-auto transition dark:bg-neutral-700",
        className,
      )}
    >
      {children}
  ...
  return (
    <button
      className={clsx(
        ...
        isActive
          ? "bg-neutral-200 dark:bg-neutral-800 dark:hover:bg-neutral-800"
...
Tabs.Content = ({ value, className, as, keepAlive = true, children }: TabsContentProps) => {
  ...
  return <div className={clsx(!isVisible && "hidden!", className)}>{children}</div>;
};
