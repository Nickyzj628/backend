				...
				onMessage: (event, ws) => {
					try {
						...
						const { event: eventName, payload } =
							validation.output as WebSocketMessage;
						...
						switch (eventName) {
							...
							case "roomMessage": {
								const result = roomService.sendRoomMessage(
									ws,
									wsId,
									payload as RoomMessagePayload,
								);
								if (!result.success) {
									sendError(result.error);
								}
							...
							case "videoSyncResponse":
								roomService.responseSync(
									ws,
									wsId,
									payload as VideoSyncResponsePayload,
								);
								break;
							default:
