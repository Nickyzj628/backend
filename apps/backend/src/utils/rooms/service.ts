...
export class RoomService {
   ...
   * 向房间内广播消息（排除发送者）
   */
  broadcastToRoom(roomCode: string, message: WSMessage, senderId?: string): void {
   ...
   * 加入房间
   */
  joinRoom(ws: WS, userId: string, payload: JoinRoomPayload): OperationResult<WSMessage> {
   ...
   * 发送房间消息
   */
  sendRoomMessage(ws: WS, userId: string, payload: RoomMessagePayload): OperationResult {
  ...
  seekVideo(ws: WS, userId: string, time: number): void {
    ...
    this.broadcastToRoom(userData.roomCode, { event: "seeked", payload: time }, userId);
  }
  ...
  changeRate(ws: WS, userId: string, rate: number): void {
    ...
    this.broadcastToRoom(userData.roomCode, { event: "rateChanged", payload: rate }, userId);
  }
  ...
  changeEpisode(ws: WS, userId: string, ep: number): void {
    ...
    this.broadcastToRoom(userData.roomCode, { event: "epChanged", payload: ep }, userId);
  }
   ...
   * 响应同步请求，发送视频状态
   */
  responseSync(ws: WS, userId: string, payload: VideoInfo & { targetId: string }): void {
    const userData = this.userMap.get(userId);
    if (!userData) return;
